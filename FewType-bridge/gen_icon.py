# -*- coding: utf-8 -*-
"""按 icon/FewType.svg 渲染 FewType 全套图标（ICO 多尺寸 + 各尺寸 PNG）。

输出三处（与代码引用路径一一对应）：
  - FewType-bridge/FewType.ico            窗口/任务栏/exe 内嵌（主）
  - FewType-bridge/icon/                  托盘备用 ico + 16/32/48/128/256 PNG + 512 源图
  - FewType-extension/icon/               浏览器扩展图标（manifest 引用 16/32/48/128）+ 512 源图
会删除无引用的遗留大图 FewType.png（1.1MB）。
"""
import math
import numpy as np
...
ROOT = Path(r"D:\Documents\FewType")
BRIDGE = ROOT / "FewType-bridge"
EXT = ROOT / "FewType-extension"

S = 512
ICO_SIZES = [(16, 16), (24, 24), (32, 32), (48, 48), (64, 64), (128, 128), (256, 256)]
PNG_SIZES = (16, 32, 48, 128, 256)

# ============ 新 SVG（FewType.svg v2，墨绿/翡翠绿主题）参数 ============
...
def emit(img, base: Path, ico: bool, png: bool):
    """输出全套到指定目录；删除无引用的遗留 FewType.png。"""
    base.mkdir(parents=True, exist_ok=True)
    if ico:
        img.save(base / "FewType.ico", format="ICO", sizes=ICO_SIZES)
    if png:
        for px in PNG_SIZES:
            img.resize((px, px), Image.Resampling.LANCZOS).save(base / f"FewType-{px}.png")
        img.save(base / "FewType-512.png")
    legacy = base / "FewType.png"
    if legacy.exists():
        legacy.unlink()
...
def main():
    img = build()
    print("输出 FewType 图标（512 源图渲染，墨绿/翡翠绿新主题）:")
    emit(img, BRIDGE, ico=True, png=False)          # 根：仅 ico
    print("  [bridge 根] FewType.ico (7 尺寸)")
    emit(img, BRIDGE / "icon", ico=True, png=True)  # icon/：ico + 全部 PNG
    print("  [bridge/icon] FewType.ico + 16/32/48/128/256.png + 512.png")
    emit(img, EXT / "icon", ico=True, png=True)     # extension/icon/：全部
    print("  [extension/icon] FewType.ico + 16/32/48/128/256.png + 512.png")
    print("OK 全套图标已更新")
