...
```bat
magick convert FewType-16.png FewType-32.png FewType-48.png FewType-256.png FewType.ico
```
...
打包后：`dist\FewType-bridge.exe` 在资源管理器中即显示该图标。
...
1. 与 exe 同目录的 `FewType.ico` / `icon/FewType-32.png` 等
...
运行时图标从 **exe 内打包的 FewType.ico**（`--add-data`）读取，发布单 exe 即可，不强制 exe 旁再放 ico。
