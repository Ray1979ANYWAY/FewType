...
- **语音打字（STT）** — 按快捷键说话，文字直接落在光标所在处——任意窗口通用。逐字模式忠实记录你所说的内容，不经 LLM 改写。
...
![VoxEcho 主界面](docs/screenshots/main_en.png)

### 🚀 快速开始

1. 从 [Releases](https://github.com/Ray1979ANYWAY/VoxEcho/releases) 下载最新安装版或便携版。
2. 运行 VoxEcho，打开**设置**，填入服务商的 API Key。
3. 按下快捷键（可在设置中自定义——Ctrl+Win 或双击 Ctrl）说话，文字将输入到光标所在处。
4. 需要电子书朗读时，将 `VoxEcho-extension` 文件夹作为"已解压的扩展程序"加载到 Chrome/Edge——不读电子书可跳过此步。
