...
def _show_voxecho_dialog(parent, title, message):
    """VoxEcho 统一风格的信息弹窗（暗色主题 + 翡翠绿强调色）。
...
_TEXTS = {
    "en": {
        "title": "VoxEcho Voice Processing Platform",
        "heading": "VoxEcho voice processing platform",
    ...
    "zh-CN": {
        "title": "VoxEcho 语音处理平台",
        "heading": "VoxEcho 语音处理平台",
    ...
    "zh-TW": {
        "title": "VoxEcho 語音處理平台",
        "heading": "VoxEcho 語音處理平台",
...
def run_gui(test_hook=None):
    ...
    root._voxecho_sponsor_img = sponsor_img  # 防止图片被 GC 回收

    # 关于按钮（微型 ℹ）：紧贴「支持 VoxEcho」右侧
    info_btn = tk.Label(
    ...
    root._voxecho_toggle_log = _toggle_log  # 测试钩子：回归冒烟直接调用
    poll_logs()
    ...
    def _pick_scene(key: str):
        ...
        _update_tabs()
        show_scene()
    root._voxecho_pick_scene = _pick_scene  # 测试钩子：回归验证直接调用（后台窗口 event_generate 不可靠）
    ...
    def do_preview():
        try:
            ...
            if not src:
                _show_voxecho_dialog(root, _pd("提示", "Notice"), _pd("请先输入或粘贴文本", "Enter or paste text first"))
                return
    ...
    def _tts_fail(e: Exception):
        msg = str(e)
        if "404" in msg:
            msg += _pd("（服务版本过旧，请退出旧版 VoxEcho-bridge 后重试）", " (old bridge version, exit the old VoxEcho-bridge and retry)")
    ...
    _update_hotkey_btn()
    root._voxecho_update_hotkey = _update_hotkey_btn  # 供 _restart_hotkey 调用更新按钮文字
    ...
    root._voxecho_hud_show = _stt_banner_show
    root._voxecho_hud_hide = _stt_banner_hide
    root._voxecho_hud_wave_start = _stt_wave_start
    root._voxecho_hud_wave_stop = _stt_wave_stop
    root._voxecho_hud_banner = _stt_banner
    root._voxecho_hud_recorder = _stt_recorder
    root._voxecho_hud_set_alpha = _hud_set_alpha
    ...
    def _stt_finish():
        ...
        def work():
            try:
                ...
                if mp3:
                    tmp = Path(tempfile.gettempdir()) / "voxecho_stt.mp3"
                    tmp.write_bytes(mp3)
                else:
                    tmp = Path(tempfile.gettempdir()) / "voxecho_stt.wav"
    ...
    def _update_hotkey_hint():
        ...
        两个场景各自定义 _update_hotkey_btn 并存到 root._voxecho_update_hotkey，
        这里统一调用，避免 NameError。"""
        fn = getattr(root, "_voxecho_update_hotkey", None)
        if callable(fn):
            try:
                fn()
            except Exception:
